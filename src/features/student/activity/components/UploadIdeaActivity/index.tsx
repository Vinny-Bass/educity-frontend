...
export interface UploadIdeaActivityProps {
  title?: string;
  description?: string;
  activityKey: string;
}

export default function UploadIdeaActivity({
  title = "Share your team's big idea",
  description = "Capture the story behind your idea so your teammates can build on it.",
  activityKey,
}: UploadIdeaActivityProps) {
  const auth = useAuth();
  ...
  useEffect(() => {
    const trimmedIdea = teamIdea.trim();
    const isValid = trimmedIdea.length > 0;
  ...
  }, [teamIdea, setIsCurrentActivityCompleted, setCompletionPayload]);
        ...
        <Card>
          <CardHeader className="rounded-[20px] bg-[#F3ECFF] px-5 py-4">
            <CardTitle className="font-baloo text-[26px] font-normal text-[#0E0420]">
              What is your team's big idea?
            </CardTitle>
          </CardHeader>

          <CardContent className="p-6 space-y-6">
            <div className="w-full max-w-[520px] items-start bg-[#F3F3F3] border-2 border-[#D4D4D4] rounded-[20px] flex flex-col gap-[10px] justify-center p-[20px_24px]">
              <div className="flex items-center gap-[10px] flex-wrap">
                {myTeam.enrollments.map((enrollment) => (
                  <div
                    key={enrollment.student.id}
                    className="items-center bg-[#EBEBEB] rounded-[10px] inline-flex flex-none gap-[10px] justify-center px-[14px] py-[10px]"
                  >
                    <div className="text-[#5A5564] font-baloo-2 text-[20px] font-medium tracking-normal leading-normal -mt-[1px]">
            ...
            <div className="space-y-2">
              <div className="w-full max-w-[520px] border-2 border-[#F3F3F3] rounded-[20px] p-6">
                <textarea
                  placeholder="Describe your idea so everyone can rally behind it"
                  value={teamIdea}
                  onChange={(e) => setTeamIdea(e.target.value)}
                  className="font-baloo-2 text-[20px] font-medium text-[#0E0420] border-none shadow-none focus-visible:outline-none focus-visible:ring-0 h-[200px] w-full resize-none bg-transparent placeholder:text-[#C6C4CB]"
                />
              </div>
            </div>
          </CardContent>
